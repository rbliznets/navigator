...
class CameraMachine(val id: Int) : StateMachine<CameraState, CameraAction> {
    ...
    init {
        ...
        handler = object : Handler(handlerThread.looper) {
            override fun handleMessage(msg: Message) {
                transition(msg.obj as CameraAction, id)
            }
        }
    }
    ...
    override fun transition(action: CameraAction, id: Int) {
        val state = this.state

        when {
            state is CameraState.WaitingStart && action is CameraAction.Start -> {
                val mutableHolder = findCameraId(state, action, id)
